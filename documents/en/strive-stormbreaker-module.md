...
- [Progression](#progression)
  ...
  - [Mechanite Technology](#mechanite-technology)
    - [Biped Automaton](#biped-automaton)
...
| ----------------------------------------------------------- | ---------------------- |
| [](#)                                   | A material that amplifies the flow of magic. Can be processed into Runes and Magic Scrolls. |
...
As such, most 'modern' technology is mostly timber-based, as it is one of the few renewable resource of the floating islands. Even the Stormbreaker's Sky Ships are mostly made of strong timber, although engines and internal equipment is made of precious steel. While concrete buildings line the streets of the capital, newer constructions are made of wood and many of the surrounding smaller islands are still very much medieval. 

Radio technology exists and is fantastically powerful, enabling the transmission of both fully holographic data and small physical goods, such as food and ammunition. However, the great Storm Walls also act as barriers of the radio waves, drastically shortening their effective range beyond them. Relays beyond the storms *could* boost the signal, allowing longer range communications, but this would require significant resources. 
...
Additionally, a longer term goal is to find a way to **re-unite the world**. If there is a way to return the world to before the cataclysm, then it must be found!
...
If the Capital's population reaches `0`, the campaign is lost, as the Stormbreaker Order has failed its charge and is now doomed to slowly starve off! If drops to `100`, a critical loss of productivity has been reached! Every time Supplies are gained by the Capital, it gains `5` units less. 

The Capital **always** gains `1D10` People at the start of every session, **as long as** Supplies are above `0`. 

## The Capital's Supplies
Both food and resources are abstracted away, called Supplies and are measured numerically. The Capital starts with `50` and can store at most this amount. 

For every `30` People, at the start of every session `1` Supply is used up! 

> If the Capital's Population is `175`, you lose `6` Supplies at the start of the next session! 

In case the Supplies drop to `0`, the campaign *is not lost*, but instead the nation is gripped by famine and disease. At the start of every session:
* Lose `2D10` People.
* Lose `1D10` Unity.

## The Capital's Corruption
Since time immemorial, an otherworldly power has taken root in ours, twisting and bending the rules of reality to its whims. While for the longest time in history, the Corruption remained an esoteric phenomenon, it is now omnipresent. It turns people into monsters, food into poison and houses into death traps. And it is growing, every day. If a more effective means to fight it cannot be found, it will end the world, as we know it. 

There are ways to keep the Corruption at bay, at least locally. Incense carries vapors from holy reliquaries that contain tiny fragments of the **Sun's Splendour** - a jewel or pure sun light and the most holy relic the sun has given us - and prevent the spread of Corruption. 

The Capital starts the game with `10` **Corruption**. If this number reaches `100`, the **campaign is lost**, as the Capital breaks apart, twisting and morphing into some kind of demon-spawn, that henceforth roams the skies, relentlessly hunting and consuming all life it can find. 

At the start of every session, the Capital gains `1D6` Corruption.

## Capital Events
At the start of every session, one of these random events will be rolled for. None of the outcomes are purely good. There is always a price. 

| D12  | Event |
| ---- | ----- |
| 1    | One of the rope bridges between the islands snaps and workers plummet to their gruesome death in the clouds. Lose `1D10` People and `1D6` Supplies. |
| 2    | A factory accident wounds many workers. Their work must now be performed by other already over-worked people. Lose `1D10` Unity. |
| 3    | A factory accident kills several workers. The remaining workers go on strike! Productivity plummets. Lose `1D6` People and `1D10` Supplies. |
| 4    | A chemical lab accident releases a noxious cloud of gas over the Capital. **Choose**: 1. Order the people to stay indoors. You lose `1D6` People and gain `1D10` Corruption, but gain `5` Unity. <br> 2. Order the people to continue their work, despite the danger. You Lose `1D10` People, `1D10` Unity and gain `1D6` Corruption, but gain `1D10 + 5` Supplies. <br>3. Order the city guard to cleanse the affected districts. Lose `2D10` People and `1D10` Unity, but also reduce Corruption by `1D6`. |
| 5    | A serial killer grips the nation! People are afraid to leave their houses! Lose `1D6 + 2` People and `1D6 + 2` Unity. |
| 6    | The Cult parades through the other quarters, in a grand spectacle. Self-flagellation and creepy chants disturb the rest of the populace. Lose `1D10` Unity. |
| 7    | Some of the Cult's extremists start a small war, until their purging of heretics causes the city guard to intervene in a bloody fight. Lose `1D10` People and `1D10` Unity. |
| 8    | Amelians provide humanitarian aid and prescribe more frequent work breaks. Productivity plummets. Lose `1D10` Supplies and gain `1D6 + 3` Unity. |
| 9    | Storms loosen from the great walls of storm, ravaging the fringe islands of the realm. Crops are ruined and lives lost. Lose `1D6` Supplies and `1D10` People and gain `1D6` Corruption. |
...
| 3      |             | Improved food processing methods grants higher yields. At the start of every session, the Capital now produces `+1` Supplies. This bonus can only stack up to a maximum of `+10`. |  | Yes |
...
|        | 10          | A grand cleansing ceremony is held, reducing Corruption by `3`. |  | Yes |
...
| 5      |             | A new way to treat incense before burning causes the vapors to spread farther. The Capital suffers `3` Corruption less from events. | Medical Laboratories `2` | No |
...
The PCs can spend **Mechanite Technology Points** to gain access to special techology. 
...
Commands can be given to it verbally or non-verbally, via an input terminal accessible at its back, such as 'overwatch', 'follow me', 'stand down', 'haul this' and similar. While impressive, this machine is not flawless. **It cannot reload its weapons on its own**! Someone else has to reload its weapons for it!

* Power: Elite, CR: `10`
* Roles: Sentry, Guard
  * Can be employed for overwatch fire support, hauling and scouting, but requires verbal command input by a registered operator. 

| Agi   | Awar  | Str   | Tough | Wit   |
| ----- | ----- | ----- | ----- | ----- |
| 2     | 4     | 6     | 6     | 3     |

| BI     | Sprint | Stabil | HP  | Stamina    | Def    | Armor             |
| ------ | ------ | ------ | --- | ---------- | ------ | ----------------- |
| 9      | 4      | 3      | 60  | Does not tire | 2D  | `10` Slashing; `8` Piercing; `8` Bludgeoning; Poison immune; |

**Traits**: [Fortified Mind](./strive-core.md#fortified-mind--4-points-1-cr); 

**Skills**:
* Special Weapons (Awar/Wit):
  * Level: `4`, AP: `2`
* Shield: Level `3`
...
* Power: Feeble, CR: `4`
* Roles: Sentry, Guard
  * By itself, a very weak foe that is to be employed as a warning, en-masse or in conjunction with other, more dangerous foes. 
* Behavior: 
  * Roams around or floats fixated upon a specific location. 
  * When it spots foes, it emits a shrill and ghostly shriek that attracts others of its kind. 
